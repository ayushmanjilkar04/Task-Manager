      ...
      tasks.forEach((task) => {
        taskList.innerHTML += `
                <div id="tasks"><li>
                    ...
                    <button id="complete-btn" onclick="completeTask('${task._id}')">Complete</button>
...
function deleteTask(id) {
  fetch(`${API}/delete-task/${id}`, { method: "DELETE" }).then(() =>
    loadTasks(),
  );
}

function completeTask(id) {
  fetch(`${API}/update-status/${id}`, { method: "PUT" }).then(() =>
    loadTasks(),
  );
}
